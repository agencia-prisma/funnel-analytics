...
const visitorId = '71000000-0000-4000-8000-000000000002';
const sessionId = '71000000-0000-4000-8000-000000000003';

function session(): SessionFactV1 {
  return {
    workspace_id: workspaceId,
    pixel_id: '71000000-0000-4000-8000-000000000004',
    session_id: sessionId,
    visitor_id: visitorId,
    session_partition_month: 202609,
    session_started_at: '2026-09-04T12:00:00.000Z',
    last_activity_at: '2026-09-04T12:00:30.000Z',
    duration_seconds: 30,
    event_count: 2,
    page_view_count: 1,
    ...
    test_mode: false,
    first_event_id: '71000000-0000-4000-8000-000000000101',
    last_event_id: '71000000-0000-4000-8000-000000000102',
    max_received_at: '2026-09-04T12:00:31.000Z',
    session_version: '5',
    updated_at: '2026-09-04T12:00:31.000Z',
  };
}
...
function message(trace: string[]): JourneyQueueMessageLike {
  return {
    body: {
      envelope_version: 1,
      request_id: '71000000-0000-4000-8000-000000000201',
      generated_at: '2026-09-04T12:01:00.000Z',
      workspace_id: workspaceId,
...
describe('Journey → Funnel recompute trigger', () => {
  it('publishes the persisted Journey before acknowledging upstream', async () => {
    const trace: string[] = [];
    const publisher: FunnelRecomputeProducer = {
      async send(envelope) {
        trace.push('funnel-publish');
        ...
        expect(envelope.deleted_journey_ids).toEqual([]);
      },
    };
  ...
  it('retries upstream when the downstream funnel publish fails', async () => {
    const trace: string[] = [];
    const input = message(trace);
    const consume = createJourneyConsumer({
      repository: repository(trace),
      ...
      funnelPublisher: {
        async send() {
          trace.push('funnel-publish-failed');
    ...
    expect(trace).toContain('funnel-publish-failed');
